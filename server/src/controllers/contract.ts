...
export const createContract = async (req: Request, res: Response) => {
  try {
    ...
    const { case_name, contract_number, contract_amount, note, installments } = snakeCaseData;
    ...
    if (installments && installments.length > 0) {
      ...
      const { error: itemsError } = await supabase.from('Installments').insert(installmentItems);
...
export const updateContract = async (req: Request, res: Response) => {
  try {
    ...
    // 轉換請求數據為 snake_case
    const snakeCaseData = snakecaseKeys(req.body, { deep: true });
    const { case_id, contract_number, contract_amount, note, installments } = snakeCaseData;
    ...
    if (installments && installments.length > 0) {
      ...
      const { error: insertError } = await supabase.from('Installments').insert(installmentItems);
