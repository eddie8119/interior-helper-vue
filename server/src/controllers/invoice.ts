...
import { InvoiceDetail, InvoiceForList, InvoiceItemInsert } from '@/types/invoice';
...
export const getInvoices = async (req: Request, res: Response) => {
  try {
    ...
    if (year && month) {
      ...
      // 計算下個月的第一天
      const nextMonth = new Date(parseInt(year as string), parseInt(month as string), 1);
...
export const createInvoice = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 驗證必要欄位
    if (!company || !case_name || !sales_amount || !total_amount || !issue_date || !due_date) {
    ...
    // 步驟 4: 創建發票項目
    const invoiceItems: InvoiceItemInsert[] = invoice_items.map((item: any) => ({
      user_id: userId,
    ...
    const { error: itemsError } = await supabase.from('InvoiceItems').insert(invoiceItems);
...
export const updateInvoice = async (req: Request, res: Response) => {
  try {
    ...
    if (invoice_number !== undefined) invoiceData.invoice_number = invoice_number;
