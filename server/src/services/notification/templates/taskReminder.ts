...
export const generateTaskReminderTemplate = (task: Record<string, unknown>): string => {
  const reminderTime = task.reminder_date_time ? formatDateTime(task.reminder_date_time as string | number | Date) : '未設置';
